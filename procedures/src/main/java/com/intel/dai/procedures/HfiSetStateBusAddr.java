...
/**
 * Handle the database processing that is necessary when setting a node's Hfi's state and PCIE BusAddr.
 *
 *  Input parameter:
...
public class HfiSetStateBusAddr extends VoltProcedure {
    ...
    public final SQLStmt update = new SQLStmt("UPDATE Hfi SET State=?, BusAddr=?, DbUpdatedTimestamp=?, LastChgTimestamp=?, LastChgAdapterType=?, LastChgWorkItemId=? WHERE NodeLctn=? AND Lctn=?;");
    ...
    public long run(String sNodeLctn, String sComponentLctn, String sNewState, String sNewBusAddr, long lTsInMicroSecs, String sReqAdapterType, long lReqWorkItemId) throws VoltAbortException
    {
        ...
        if (aData[0].getRowCount() == 0) {
            throw new VoltAbortException("HfiSetStateBusAddr - there is no entry in the Hfi table for the specified " +
