...
public class ComputeNodeSetOwner extends ComputeNodeCommon {
    ...
    public final SQLStmt updateNode = new SQLStmt("UPDATE ComputeNode SET Owner=?, DbUpdatedTimestamp=?, LastChgTimestamp=?, LastChgAdapterType=?, LastChgWorkItemId=? WHERE Lctn=?;");
    ...
    public long run(String sNodeLctn, String sNodeNewOwner, long lTsInMicroSecs, String sReqAdapterType, long lReqWorkItemId) throws VoltAbortException {
        ...
        if (sNodeNewOwner.equals("G")) {
            // a compute node should never be owned by the general system (this may prove to be an incorrect assumption).
            throw new VoltAbortException("ComputeNodeSetOwner - an invalid new owner was specified (" + sNodeNewOwner + ") for this " +
        ...
        if (aNodeData[0].getRowCount() == 0) {
            throw new VoltAbortException("ComputeNodeSetOwner - there is no entry in the ComputeNode table for the specified " +
                                         "node lctn(" + sNodeLctn + ") - ReqAdapterType=" + sReqAdapterType + ", ReqWorkItemId=" + lReqWorkItemId + "!");
        }
        ...
        if (lTsInMicroSecs > lCurRecordsTsInMicroSecs) {
            ...
            aNodeData[0].advanceRow();
//            System.out.println("ComputeNodeSetOwner - " + sNodeLctn + " - OUT OF ORDER" +
...
//                               " - CurRecordsTsInMicroSecs=" + lCurRecordsTsInMicroSecs + ", CurRecordsOwner=" + sCurRecordsOwner + "!");
            ...
            if (aNodeData[0].getRowCount() == 0) {
//                System.out.println("ComputeNodeSetOwner - there is no row in the history table for this lctn (" + sNodeLctn + ") that is older than the time specified on this request, "
