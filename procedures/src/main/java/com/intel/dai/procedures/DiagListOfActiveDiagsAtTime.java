...
public class DiagListOfActiveDiagsAtTime extends VoltProcedure {

    public final SQLStmt selectWithTimeSql = new SQLStmt(
        "SELECT * FROM Diag " +
        "WHERE (StartTimestamp<=? AND ((EndTimestamp IS NULL) OR (EndTimestamp>?))) " +
    ...
    public VoltTable run(TimestampType sEndingTimestamp) throws VoltAbortException {
        ...
        if (sEndingTimestamp != null)
            voltQueueSQL(selectWithTimeSql, sEndingTimestamp, sEndingTimestamp);
        // Ending time was NOT specified - use current time
        else
            voltQueueSQL(selectWithTimeSql, this.getTransactionTime(), this.getTransactionTime());
