...
/**
 * Handle the database processing that is necessary when setting a node's Processor's state.
 *
 *  Input parameter:
...
public class ProcessorSetState extends VoltProcedure {
    public final SQLStmt select = new SQLStmt("SELECT * FROM Processor WHERE NodeLctn=? AND Lctn=?;");
    ...
    public final SQLStmt update = new SQLStmt("UPDATE Processor SET State=?, DbUpdatedTimestamp=?, LastChgTimestamp=?, LastChgAdapterType=?, LastChgWorkItemId=? WHERE NodeLctn=? AND Lctn=?;");
    ...
    {
        //----------------------------------------------------------------------
        // Grab the current record for this Lctn out of the "active" table (Processor table).
        ...
        if (aData[0].getRowCount() == 0) {
            throw new VoltAbortException("ProcessorSetState - there is no entry in the Processor table for the specified " +
        ...
        voltQueueSQL(insertHistory
                    ...
                    ,sComponentLctn                         // Lctn
                    ,sNewState                              // State
                    ,aData[0].getString("SocketDesignation")
