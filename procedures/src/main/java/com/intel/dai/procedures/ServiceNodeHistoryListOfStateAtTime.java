 ...
 *
 *  Flows
 *      1)  If EndingTimestamp IS null - then just return the states for each lctn in descending order from the ServiceNode table (not the history table)
 ...
 *              All the rest of the entries will be the CURRENT state for each of the lctns in the machine (using the ServiceNode table).
 *          b)  There will be exactly 1 + number of machine service node lctns.
 *          c)  The states will be returned in descending lctn order.
 *      2)  If EndingTimestamp is not null and StartingTimestamp IS null
 *          a)  The entries will consist of 1 string
 *              - The Lctn's state
 *          b)  The entries will be for each lctn's record with the "newest" timestamp that is <= the specified ending timestamp (using the ServiceNode_History table).
 ...
 *              - The lctn
 *              - The Lctn's state
 *          b)  The entries will be for each lctn's record with the "newest" timestamp that are >= the specified starting timestamp AND <= the specified ending timestamp (using the ServiceNode_History table).
 *          c)  There will be at most one entry for each lctn in the machine.
 *          d)  The states will be returned in descending lctn AND timestamp order.
 *
 *  echo "execute ServiceNodeHistoryListOfStateAtTime '2017-03-15 19:15:06.377002', '2017-03-15 19:15:06.377001';" | sqlcmd
 *  echo "execute ServiceNodeHistoryListOfStateAtTime '2017-03-15 19:15:06.377002', null;" | sqlcmd
 *  echo "execute ServiceNodeHistoryListOfStateAtTime '2017-03-15 19:15:06.377002', '';" | sqlcmd
 *
 */

public class ServiceNodeHistoryListOfStateAtTime extends VoltProcedure {

    public final SQLStmt selectCurrentNodeStatesSql = new SQLStmt(
        "SELECT State from ServiceNode Order By SequenceNumber;"
    );
    ...
    public VoltTable run(TimestampType sEndingTimestamp, TimestampType sStartingTimestamp) throws VoltAbortException {
        ...
        if (bEndingTsSpecified == false) {
            //------------------------------------------------------------------
            // Get the list of CURRENT states for the nodes in the machine (use the ServiceNode table, not the ServiceNode_History table).
            //------------------------------------------------------------------
