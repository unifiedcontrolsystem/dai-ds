...
/**
 * Handle the database processing that is necessary when there is new BIOS info that needs to be saved in the DB for the specified node.
 *
 *  Input parameter:
...
public class ComputeNodeSaveBiosInfo extends ComputeNodeCommon {
    ...
    public final SQLStmt insertNodeHistory = new SQLStmt(
            "INSERT INTO ComputeNode_History(" +
                "Lctn, SequenceNumber, State, HostName, BootImageId, Environment, IpAddr, " +
                "MacAddr, BmcIpAddr, BmcMacAddr, BmcHostName, DbUpdatedTimestamp, LastChgTimestamp, " +
                "LastChgAdapterType, LastChgWorkItemId, Owner, Aggregator, InventoryTimestamp, WlmNodeState, ConstraintId, ProofOfLifeTimestamp) " +
            "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);"
    );


    // Update the record for this Lctn in the "active" node table.
    ...
    void insertRecordIntoNodeInventoryHistoryTable(String sNodeLctn, long lNewRecsLastChgTimestampTsInMicroSecs, String sInventoryInfo, String sSernum, String sBiosInfo)
    {
    ...
    }   // End insertRecordIntoNodeInventoryHistoryTable(String sNodeLctn, long lNewRecsLastChgTimestampTsInMicroSecs, String sInventoryInfo, String sNewSernum, String sBiosInfo)

    // Insert a "history" record for these updates into the node's history table
    ...
    void insertRecordIntoNodeHistoryTable(VoltTable nodeData, long lNewRecsLastChgTimestampTsInMicroSecs, String sReqAdapterType, long lReqWorkItemId)
    {
        voltQueueSQL(insertNodeHistory
                    ...
                    ,nodeData.getString("Environment")
                ...
                throws VoltAbortException
    {
        ...
        //----------------------------------------------------------------------
        VoltTable[] aNodeData = getComputeNode(sParmNodeLctn);
        VoltTable nodeData = aNodeData[0];
        if (nodeData.getRowCount() == 0) {
            throw new VoltAbortException("ComputeNodeSaveBiosInfo - there is no entry in the ComputeNode table for the " +
        ...
        if (lNewRecsLastChgTimestampTsInMicroSecs <= lCurRecsLastChgTimestampTsInMicroSecs)
            lNewRecsLastChgTimestampTsInMicroSecs = ensureHaveUniqueComputeNodeLastChgTimestamp(sParmNodeLctn, lNewRecsLastChgTimestampTsInMicroSecs, lCurRecsLastChgTimestampTsInMicroSecs);
        ...
        if (lNewRecsLastChgTimestampTsInMicroSecs > lCurRecsLastChgTimestampTsInMicroSecs) {
            ...
            // Insert a "history" record for this node's inventory information.
            insertRecordIntoNodeInventoryHistoryTable(sParmNodeLctn, lNewRecsLastChgTimestampTsInMicroSecs, sOldInventoryInfo, sOldSernum, sParmBiosInfo);
            ...
            //------------------------------------------------------------------
            aNodeData = getComputeNodeFromHistoryWithPrecedingTime(sParmNodeLctn, lNewRecsLastChgTimestampTsInMicroSecs);
            ...
            if (aNodeData[0].getRowCount() == 0) {
                System.out.println("ComputeNodeSaveBiosInfo - there is no row in the node history table for this lctn (" + sParmNodeLctn + ") that is older than the time specified on this request, "
