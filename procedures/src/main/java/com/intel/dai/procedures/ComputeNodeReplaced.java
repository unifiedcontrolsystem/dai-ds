...
/**
 * Handle the database processing that is necessary when a node has been "replaced".
 *
 *  Input parameter:
...
public class ComputeNodeReplaced extends ComputeNodeCommon {
                ...
                throws VoltAbortException
    {
        ...
        long lRc = 0;
        if (sNewSernum == null) {
            throw new VoltAbortException("ComputeNodeReplaced - no serial number was provided for the " +
        ...
        if (nodeData.getRowCount() == 0) {
            throw new VoltAbortException("ComputeNodeReplaced - there is no entry in the ComputeNode table for the " +
                    "specified node lctn(" + sNodeLctn + ") - ReqAdapterType=" + sReqAdapterType + ", ReqWorkItemId=" +
                    lReqWorkItemId + "!");
        }
        ...
        if (lNewRecsLastChgTimestampTsInMicroSecs > lCurRecsLastChgTimestampTsInMicroSecs) {
            ...
            if (aNodeData[0].getRowCount() == 0) {
                System.out.println("ComputeNodeReplaced - there is no row in the history table for this lctn (" + sNodeLctn + ") that is older than the time specified on this request, "
