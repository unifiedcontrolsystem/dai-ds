...
/*
 * Handle the database processing that is necessary when setting a node's Accelerator's state.
 *
 *  Input parameter:
...
public class AcceleratorSetState extends VoltProcedure {
    ...
    public final SQLStmt update = new SQLStmt("UPDATE Accelerator SET State=?, DbUpdatedTimestamp=?, LastChgTimestamp=?, LastChgAdapterType=?, LastChgWorkItemId=? WHERE NodeLctn=? AND Lctn=?;");
    ...
    public long run(String sNodeLctn, String sComponentLctn, String sNewState, long lTsInMicroSecs, String sReqAdapterType, long lReqWorkItemId) throws VoltAbortException
    {
        ...
        if (aData[0].getRowCount() == 0) {
            throw new VoltAbortException("AcceleratorSetState - there is no entry in the Accelerator table for the specified " +
