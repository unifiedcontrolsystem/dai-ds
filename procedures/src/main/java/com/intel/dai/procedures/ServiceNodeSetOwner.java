...
/**
 * Handle the database processing that is necessary when setting which subsystem is the owner of this node.
 *
 *  Input parameter:
 *      String  sNodeLctn               = string containing the Lctn of the node
 *      String  sNodeNewOwner           = string containing the new owner value for the above node location
 *      long    lTsInMicroSecs          = Time that this node owner changed
...
public class ServiceNodeSetOwner extends ServiceNodeCommon {
    ...
    public final SQLStmt insertNodeHistory = new SQLStmt(
            ...
            "(Lctn, SequenceNumber, State, HostName, BootImageId, IpAddr, MacAddr, BmcIpAddr, BmcMacAddr, BmcHostName, DbUpdatedTimestamp, LastChgTimestamp, LastChgAdapterType, LastChgWorkItemId, Owner, Aggregator, InventoryTimestamp, ConstraintId, ProofOfLifeTimestamp) " +
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"
    );

    public final SQLStmt updateNode = new SQLStmt("UPDATE ServiceNode SET Owner=?, DbUpdatedTimestamp=?, LastChgTimestamp=?, LastChgAdapterType=?, LastChgWorkItemId=? WHERE Lctn=?;");



    public long run(String sNodeLctn, String sNodeNewOwner, long lTsInMicroSecs, String sReqAdapterType, long lReqWorkItemId) throws VoltAbortException {
        // Ensure that the specified new owner is not nonsensical.
        if (sNodeNewOwner.equals("W")) {
            // a service node should never be owned by the WLM.
            throw new VoltAbortException("ServiceNodeSetOwner - an invalid new owner was specified (" + sNodeNewOwner + ") for this " +
        ...
        if (aNodeData[0].getRowCount() == 0) {
            throw new VoltAbortException("ServiceNodeSetOwner - there is no entry in the ServiceNode table for the specified " +
                                         "node lctn(" + sNodeLctn + ") - ReqAdapterType=" + sReqAdapterType + ", ReqWorkItemId=" + lReqWorkItemId + "!");
        }
        ...
        if (lTsInMicroSecs > lCurRecordsTsInMicroSecs) {
            ...
            aNodeData[0].advanceRow();
            System.out.println("ServiceNodeSetOwner - " + sNodeLctn + " - OUT OF ORDER" +
                               " - ThisRecsTsInMicroSecs="   + lTsInMicroSecs           + ", ThisRecsOwner="   + sNodeNewOwner +
                               " - CurRecordsTsInMicroSecs=" + lCurRecordsTsInMicroSecs + ", CurRecordsOwner=" + sCurRecordsOwner + "!");
            ...
            if (aNodeData[0].getRowCount() == 0) {
                System.out.println("ServiceNodeSetOwner - there is no row in the history table for this lctn (" + sNodeLctn + ") that is older than the time specified on this request, "
        ...
        if (bUpdateCurrentlyActiveRow) {
            voltQueueSQL(updateNode, sNodeNewOwner, this.getTransactionTime(), lTsInMicroSecs, sReqAdapterType, lReqWorkItemId, sNodeLctn);
        }
        ...
        voltQueueSQL(insertNodeHistory
                    ...
                    ,sReqAdapterType
                    ,lReqWorkItemId
                    ,sNodeNewOwner                              // New owning subsystem
