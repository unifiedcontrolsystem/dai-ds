...
public class DimmSetState extends VoltProcedure {
    ...
    public final SQLStmt update = new SQLStmt("UPDATE Dimm SET State=?, DbUpdatedTimestamp=?, LastChgTimestamp=?, LastChgAdapterType=?, LastChgWorkItemId=? WHERE NodeLctn=? AND Lctn=?;");
    ...
    public long run(String sNodeLctn, String sComponentLctn, String sNewState, long lTsInMicroSecs, String sReqAdapterType, long lReqWorkItemId) throws VoltAbortException
    {
        ...
        if (aData[0].getRowCount() == 0) {
            throw new VoltAbortException("DimmSetState - there is no entry in the Dimm table for the specified " +
        ...
        voltQueueSQL(insertHistory
                    ...
                    ,sComponentLctn                     // Lctn
                    ,sNewState                          // State
                    ,aData[0].getLong("SizeMB")
                    ,aData[0].getString("ModuleLocator")
                    ,aData[0].getString("BankLocator")
