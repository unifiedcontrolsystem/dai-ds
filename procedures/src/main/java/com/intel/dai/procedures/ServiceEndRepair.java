...
public class ServiceEndRepair extends VoltProcedure {
    ...
    public final SQLStmt updateServiceSql = new SQLStmt(
                    "UPDATE ServiceOperation SET UserStoppedService = ?, State = ?, Status = ?, StopTimestamp = ?, StopRemarks = ?, DbUpdatedTimeStamp = CURRENT_TIMESTAMP WHERE ServiceOperationId = ? AND LCTN = ?;" );
