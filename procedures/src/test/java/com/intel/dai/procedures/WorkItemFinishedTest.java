...
public class WorkItemFinishedTest {
    class MockWorkItemFinished extends WorkItemFinished {
        @Override
    ...
    @Test
    public void run() {
        MockWorkItemFinished proc = new MockWorkItemFinished();
        proc.tables_.add(buildTable());
        proc.tables_.add(buildTable());
        proc.run("RAS", 9998L, "Results");
    }
