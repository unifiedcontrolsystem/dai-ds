...
public class WorkItemRequeueZombiesTest {
    class MockWorkItemRequeueZombies extends WorkItemRequeueZombies {
        @Override
    ...
    private VoltTable[] buildTable(boolean noRows, String state, String notify) {
        ...
        if(!noRows)
            result[0].addRow("WLM", 999L, new TimestampType(Date.from(Instant.now())), "Queue", "WLM", "Work",
    ...
    @Test
    public void run() {
        MockWorkItemRequeueZombies proc = new MockWorkItemRequeueZombies();
