...
public class ComputeNodeSaveIpAddrTest {
    class MockComputeNodeSaveIpAddr extends ComputeNodeSaveIpAddr {
        @Override
        ...
        public VoltTable[] voltExecuteSQL(boolean value) {
            ...
            if(!zeroRows) {
                result[0].addRow(new TimestampType(5000000L), "Good", "Lctn", 0L, "HostName",
                        ...
                        "rich", "Agg01", new TimestampType(Date.from(Instant.ofEpochMilli(40L))), "A");
            }
            return result;
        }
    ...
    @Test
    public void run() {
        MockComputeNodeSaveIpAddr proc = new MockComputeNodeSaveIpAddr();
    ...
    public void run3() {
        MockComputeNodeSaveIpAddr proc = new MockComputeNodeSaveIpAddr();
        long micro = Instant.now().toEpochMilli() * 1000;
        proc.run("Lctn", "IpAddrNot", micro, "RAS", 9999L);
    }
