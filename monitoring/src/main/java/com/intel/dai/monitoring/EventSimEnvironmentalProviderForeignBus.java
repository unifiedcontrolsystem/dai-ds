 ...
 */
public class EventSimEnvironmentalProviderForeignBus implements NetworkListenerProvider, Initializer {
    public EventSimEnvironmentalProviderForeignBus(Logger logger) {
        log_ = logger;
        parser_ = ConfigIOFactory.getInstance("json");
        if(parser_ == null) throw new RuntimeException("Failed to create a JSON parser instantiating class " +
                EventSimEnvironmentalProviderForeignBus.class.getCanonicalName());
    ...
    public void actOnData(CommonDataFormat data, NetworkListenerConfig config, SystemActions systemActions) {
        ...
        if(data.haveSummary()) {
            ...
            log_.debug("*** CommonDataFormat: %s", data.toString());
