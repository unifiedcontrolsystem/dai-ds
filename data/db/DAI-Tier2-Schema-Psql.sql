...
BEGIN
    RETURN QUERY
        ...
        WHERE lctn_prefix = ''
           OR lctn_prefix = t.Lctn
           OR position(lctn_prefix || '-' in t.Lctn) = 1
        ORDER BY t.Lctn;
    RETURN;
END
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION public.LastTimestampOfActionOnFruRaw(p_action varchar
                                                               , p_fru_id varchar) RETURNS varchar
AS
$$
DECLARE
...
CREATE OR REPLACE FUNCTION public.OccupationHistoryOfFruInHWInfo(p_fru_id varchar
                                                                , p_slot_loc varchar
...
$$
DECLARE
    fru_id             varchar   := format('(t.InventoryInfo->''HWInfo'')::json#>>''{fru/%s/fru_id, value}''',
                                           p_slot_loc);
