 ...
 *
 * Example invocation:
 *      java AdapterRas voltdbserver1,voltdbserver2,10.11.12.13  (or java AdapterRas  - this will default to using
 *      localhost)
 *
...
public class AdapterRas {
    ...
    // Constructor
    AdapterRas(AdapterInformation adapter, Logger logger, DataStoreFactory factory) {
        log_ = logger;
        adapter_ = adapter;
        factory_ = factory;
    ...
    public static void main(String[] args) throws AdapterException {
        Logger logger = LoggerFactory.getInstance("RAS", AdapterRas.class.getName(), "console");
        ...
        final AdapterInformation adapter = new AdapterInformation("RAS",AdapterRas.class.getName(), location, hostname);
        final AdapterRas obj = new AdapterRas(adapter, logger, new DataStoreFactoryImpl(servers, logger));
