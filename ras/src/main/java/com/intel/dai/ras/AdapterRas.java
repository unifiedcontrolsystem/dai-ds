            ...
            // Run the specified ControlOperation.
                ...
                switch(sTempControlOperation) {
                    ...
                    case "ErrorAndKillJobOnNode":
                        ...
                        adapter_.killJob(sTempLctn, sTempDescrName, lTempEventId, sTempControlOperation, sTempJobId, adapter_.adapterType(), workQueue_.baseWorkItemId());
                        break;
                    ...
                    case "ErrorAndPwrOffNode":
                        ...
                        adapter_.markNodeInErrorState(sTempLctn, sTempDescrName, lTempEventId, sTempControlOperation, sTempJobId, adapter_.adapterType(), workQueue_.baseWorkItemId());
                        // Power Off this node.
//                        controlHandler_.powerOffNode(sTempLctn, adapter_.mapNodeLctnToAggregator(), sTempDescrName, lTempEventId, sTempControlOperation, sTempJobId, adapter_.adapterType(), workQueue_.baseWorkItemId());
                        break;
...
//                        controlHandler_.powerOffNode(sTempLctn, adapter_.mapNodeLctnToAggregator(), sTempDescrName, lTempEventId, sTempControlOperation, sTempJobId, adapter_.adapterType(), workQueue_.baseWorkItemId());
                    ...
                    case "ErrorAndShutdownNode":
                        ...
                        adapter_.markNodeInErrorState(sTempLctn, sTempDescrName, lTempEventId, sTempControlOperation, sTempJobId, adapter_.adapterType(), workQueue_.baseWorkItemId());
                        // Shutdown this node.
//                        controlHandler_.shutdownNode(sTempLctn, adapter_.mapNodeLctnToAggregator(), sTempDescrName, lTempEventId, sTempControlOperation, sTempJobId, adapter_.adapterType(), workQueue_.baseWorkItemId());
                        break;
...
//                        controlHandler_.increaseNodeFanSpeed(sTempLctn, adapter_.mapNodeLctnToAggregator(), sTempDescrName, lTempEventId, sTempControlOperation, sTempJobId, adapter_.adapterType(), workQueue_.baseWorkItemId());
                        break;
                    ...
                    case "NodeIsPoweredOff":
                        ...
                        adapter_.markNodePoweredOff(sTempLctn, lLastChgTsInMicroSecs, adapter_.adapterType(), workQueue_.baseWorkItemId());
                        break;
...
//                        controlHandler_.powerCycleNode(sTempLctn, adapter_.mapNodeLctnToAggregator(), sTempDescrName, lTempEventId, sTempControlOperation, sTempJobId, adapter_.adapterType(), workQueue_.baseWorkItemId());
                        break;
                    case "ErrorAndResetNode":
                        // Mark this node as being in Error state.
                        adapter_.markNodeInErrorState(sTempLctn, sTempDescrName, lTempEventId, sTempControlOperation, sTempJobId, adapter_.adapterType(), workQueue_.baseWorkItemId());
                        // Ensure that this node is not in "reset recursion".
                        if (!isNodeInResetRecursion(sTempLctn)) {
...
//                            String[] saLctnsToReset = { sTempLctn };
//                            controlHandler_.resetNodes(saLctnsToReset, adapter_.mapNodeLctnToAggregator(), sTempDescrName, lTempEventId, sTempControlOperation, sTempJobId, adapter_.adapterType(), workQueue_.baseWorkItemId());
                            log_.debug("This node is NOT in reset recursion: Lctn=%s", sTempLctn);
                        }
                        else {
                            ...
                            adapter_.logRasEventWithEffectedJob("RasResetRecursionDetected"
                                                              ...
                                                              ,sTempLctn                              // Lctn associated with this ras event
                                                              ,sTempJobId                             // JobId of a job that was effected by this ras event
                    ...
                    case "TestControlOperation":
                        adapter_.client().callProcedure(adapter_.createHouseKeepingCallbackNoRtrnValue(adapter_.adapterType(), adapter_.adapterName(), "ErrorOnComputeNode", sTempLctn, workQueue_.baseWorkItemId())
                                                       ,"ErrorOnComputeNode"
                                                       ,sTempLctn
                                                       ,adapter_.adapterType()
                                                       ,workQueue_.baseWorkItemId());
...
//            log_.fatal("Cleaning up the array list of the nodes that are in service");
//            mNodesInServiceArraylist.clear();
//            log_.fatal("Cleaned up the array list of the nodes that are in service");
//        }
...
//            controlHandler_ = new ControlApi(adapter_.adapterName(), adapter_.adapterType(), workQueue_);
